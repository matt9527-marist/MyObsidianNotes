...
2. `C` forwards ticket to `TGS` as Message C. (Message B and Message C are the same ticket), alongside Message D, which is a server request for a certain service. 
3. `TGS` must decrypt the ticket using a shared key with `AS`. They receive a red key. 